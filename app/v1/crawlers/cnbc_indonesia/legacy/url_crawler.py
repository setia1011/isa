    ...
    try:
        ...
        for i in range(pages):
            if i < len(next_page):
                ...
                # boxes = soup.find('ul', class_='list media_rows middle thumb terbaru gtm_indeks_feed').find_all('li')
                ...
                if len(next_page) < pages:
                    # paging = soup.find('div', class_="paging text_center gtm_paging")
        ...
        # for i in urls:
            # print(i.crawled_at)
            # try:
            #     q = install_service.insert_url(crawled_at=i.crawled_at, url=i.url, source_id=i.source_id, status=i.status, db=db)
            #     db.add(q)
            #     db.commit()
            # except Exception as e:
            #     raise
        # db.bulk_insert_mappings(Urls, urls)
        # db.commit()
